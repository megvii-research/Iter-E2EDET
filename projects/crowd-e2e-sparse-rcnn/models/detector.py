...
class SparseRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.iter_num = cfg.MODEL.SparseRCNN.ITER_NUM
        # Build Backbone.
        ...
        self.auxi_proposal_features = nn.Embedding(self.num_proposals, 4 * self.hidden_dim)
    ...
    def parse_output(self, containers):
        ...
        if len(containers) <= self.watershed - 1:
            ...
            assert len(containers) >= self.watershed + self.iter_num
            prev = containers[self.watershed - 1]
            cur = containers[self.watershed]

            mask = cur_mask = cur['mask']
            cls_logits = (1 - mask) * prev['class_logits'] + mask * cur['class_logits']
            pred_bbox = (1 - mask) * prev['bboxes'] + mask * cur['bboxes']
            
            output = {'pred_logits': cls_logits, 'pred_boxes': pred_bbox}
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if self.deep_supervision:
                aux_output = [{'pred_logits': c['class_logits'], 'pred_boxes': c['bboxes'], 'container': c} for c in ctns]
                output['aux_outputs'] = aux_output
